...
export class ConversationPromptBuilder {
  ...
  rules(): string[] {
    return [
      ...
      '- [TOOL POLICY — MANDATORY] The explain_expression tool is the only way an English expression is saved to the learner\'s vocabulary book. Whenever the user asks how to say something ("how to say X", "X in English", "X 怎么说", "想学一下"), you MUST call it on the SAME turn — never translate without it, never postpone the call to a later turn.',
      '  The same rule applies when an otherwise English sentence contains a Chinese word or short phrase because the learner does not know its English expression. That embedded fragment must be explained and saved on the same turn.',
      '  Call the tool before or together with your reply, passing the expression to learn as `text` (the exact Chinese fragment is allowed) and a one-line `context`. Then reply naturally with the translation and a friendly follow-up.',
      '',
      '  user: How do you say "舒缓的节奏,有韵律"',
      '  assistant: oh, you mean "soothing, rhythmic pace". I love that kind of music too — what makes it your favorite?',
      '',
